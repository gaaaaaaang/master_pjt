...
개별 LOT·장비 기록 조회는 원문 식별자 보존, 출처별 존재 확인, 항목별 근거 범위 검증을
거친다. 동일 구조의 경로 테이블은 검증된 `UNION ALL` 계획으로 합칠 수 있다.
조회 성공과 업무 항목의 충족 여부는 별도로 반환한다. 데이터 연결 및 의미 정의 기준은
[현장 기록 조회 데이터 연결 기준](business_record_data_contract.md)에 정리했다.

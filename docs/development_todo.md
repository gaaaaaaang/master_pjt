...
## 2026-09-10 RAG 후속 고도화 (`feat/rag_adv2`)

- [x] `4ebf833` 기준 분기 및 이전 RAG 세션 이력 확인. 기존 미커밋 변경은 분리 보존.
- [x] 이슈 정합성 필터를 top-k 선정 전에 적용하고 실제 evidence 수를 trace에 기록.
- [x] KB 업데이트 검색과 Hold 보류·해제 비교 근거 누락 복구. 확장셋 필수 근거 5/7 → 7/7.
- [x] 원문 승인·기록 조건 중복 최소화, 문서 상태와 버전 관리 혼동 검증 추가.
- [x] 인용·숫자 검증 실패 시 같은 원문으로 1회 수정 후 재검증.
- [x] 최종 RAG 회귀 146개 통과. 전체 회귀 658개 통과, 기준 커밋에도 있는 SQL 복구 실패 1개 분리.
- [ ] 역할별 답변의 불필요한 일반 설명 축소, 모델 검토 안정성 및 기존 challenge 6개 누락 개선.
- [ ] 질문 요구사항별 gold answer/context, 반복 실행·다른 judge·독립 holdout 평가.

실제 평가의 모든 실행과 한계: [RAG 후속 고도화 보고서](rag_adv2_20260910.md).

### RAG 안정성 후속 검증

- [x] 고정 코드·근거로 전후 각각 6문항 × 3회 생성·검토, 모든 모델 입력·출력 보존.
- [x] RAG generation/review temperature=0 명시, 질문 기반의 간결한 주장·검토 기준 보완.
- [x] 수정 필요 실행 3/18 → 0/18, 상태 일관성은 양쪽 모두 6/6. 대체 장비의 분량 변동은 미해결.
- [x] 고정 답변 재채점으로 Ragas noncommittal의 0점 처리 원인 확인.
- [ ] 복합 절차의 답변 요건·사용 근거를 고정하는 구조화된 답변 계획과 필수 조건 평가.

분석·실측·한계: [RAG 안정성 보고서](rag_stability_20260910.md).

...
2026-09-08 08:00 KST 종료. 구현 체크포인트는 `2def792`이며 결과 정리만 후속 커밋한다.

- 단순 검색 PoC에서 하이브리드 검색·원문 인용·승인/기록 조건 보존·실제 chat 평가가 가능한
  프로토타입으로 개선했다. 토스 운영 수준과 동등하다고 검증한 것은 아니다.
- 실제 전체 chat 10문항, 일반 HTTP 3문항, 실제 loopback TCP HTTP/SSE와 변경 후 재시험을
  수행했다. 서로 다른 실행을 하나의 정답률로 합산하지 않는다.
- 마지막 PM 전체 API 재시험은 14.402초, 원문 조건 3개 보존, 인용 4개 corpus 대조 통과.
  해당 보고서의 source_sha256이 최종 코드와 일치함을 별도로 확인했다.
- 최종 로컬 Python 테스트 195 passed. 변경 파일 Ruff 통과. 웹 빌드와 인용 펼침 화면 검증 완료.
- 55청크는 시뮬레이션 자료다. 실제 승인 SOP, 독립 현장 질문/전문가 평가, 운영 부하/보안 검증,
  실제 SQL·문서 혼합 답변의 검증은 남아 있다. 인용의 존재와 답변 완전성은 별개다.
- 새 복합 질문 3건은 자동 승인 검토의 개별 전송 승인 요구로 끝까지 실제 API 미검증 상태다.
  기존 승인 질문의 실제 API 검증 완료와 구분한다.
- 검증용 rag-adv-validation의 Milvus/etcd/MinIO를 종료하고 볼륨은 보존했다.
  rag-7 heartbeat를 PAUSED로 변경했다. 별도 승인 응답이 오기 전 추가 API 호출은 없다.
...
검증 컨테이너 종료와 heartbeat PAUSED 상태를 실제 조회로 확인했다. 직전 API 실측 이후
코드는 변경하지 않았으며 테스트를 불필요하게 반복하지 않았다. 승인 대기 3문항을 제외한
이번 구현·기존 승인 범위 API 검증·원문 대조·회귀 검증·커밋·결과 기록을 마무리했다.

...
function Workspace() {
  ...
  useEffect(() => {
    ...
    loadConversations().then(saved => {
      if (cancelled) return;
      if (Array.isArray(saved) && saved.length && saved.every(c => typeof c.id === 'string' && typeof c.title === 'string' && Array.isArray(c.messages))) {
        setConversations(saved.map(c => ({ ...c, context: { fab: '', line: '', process: '', ...c.context }, messages: restoreMessages(c.messages) })));
        const ui = restoreWorkspaceUi(readLegacy(`${STORE_KEY}:ui`), saved.map(c => c.id));
        setActiveId(ui.activeId); setDrafts(ui.drafts);
      }
      setStorageReady(true);
    });
    return () => { cancelled = true; };
  }, []);
  useEffect(() => { if (!isPreview && storageReady) saveConversations(conversations).catch(() => setNotice('대화를 저장하지 못했어요. 브라우저 저장 공간과 권한을 확인해 주세요.')); }, [conversations, storageReady]);
  useEffect(() => {
    if (isPreview || !storageReady) return;
    try { localStorage.setItem(`${STORE_KEY}:ui`, JSON.stringify({ activeId, drafts })); }
    catch { setNotice('초안을 저장하지 못했어요. 새로고침 전에 입력 내용을 복사해 주세요.'); }
  }, [activeId, drafts, storageReady]);
  ...
  function patchConversation(id, update) { setConversations(current => current.map(c => c.id === id ? { ...update(c), updatedAt: Date.now() } : c)); }
  ...
  function startNew() { if (busy || !storageReady) return; if (isPreview) { window.location.href = '/'; return; } const next = newConversation(); setConversations(current => [next, ...current]); setActiveId(next.id); setSearch(''); setDrawer(null); setSidebar(false); shouldFollow.current = true; textarea.current?.focus(); }
  function choose(id) { if (busy || !storageReady) return; setActiveId(id); setDrawer(null); setSidebar(false); shouldFollow.current = true; }
  async function send(text = draft, retryId = null) {
    if (busy || !storageReady || abortRef.current || !text.trim()) return;
    ...
    try {
      ...
      await consumeSse(response, event => {
        if (event.type === 'run_started' && event.data?.conversation_id) patchConversation(conversation, c => ({ ...c, backendId: event.data.conversation_id }));
  ...
  return <div className="app-shell">
    ...
    <aside ref={sidebarRef} inert={mobile && !sidebar} className={`sidebar ${sidebar ? 'is-open' : ''}`} aria-label="대화 탐색">
      ...
      <button className="new-chat" onClick={startNew} disabled={busy || !storageReady}><Icon name="plus" size={19}/>새 대화<span>＋</span></button>
    ...
    {drawer?.type === 'guide' && <QuestionGuide initialFab={active.guide?.fab || active.context.fab} initialFlowId={active.guide?.flowId} onClose={() => setDrawer(null)} onSelect={(prompt, guide) => { patchConversation(active.id, c => ({ ...c, guide })); setDraft(prompt); setDrawer(null); setNotice('질문을 입력했어요. 내용을 확인한 뒤 보내 주세요.'); requestAnimationFrame(() => textarea.current?.focus()); }}/ >}
...
function AssistantMessage({ message, busy, canRetry, onRetry, onInspect, onFeedback, onNotice }) {
  ...
  return <article className="assistant-message" aria-label="FAB Assistant 답변"><div className="assistant-heading"><Brand small/><strong>FAB Assistant</strong>{result && <Chip status={result.status}/>}</div>
    {pending ? <div className="working-state" role="status"><span className="thinking-dots"><i/><i/><i/></span><div><strong>{progressLabel(message.events || [])}</strong><small>답변과 근거를 함께 준비하고 있어요.</small></div></div> : failed ? <div className={`error-state ${message.status === 'cancelled' ? 'cancelled' : ''}`}><Icon name="info"/><div><h3>{message.status === 'cancelled' ? '응답 수신을 중단했어요' : '답변을 가져오지 못했어요'}</h3><p>{message.error || '서버의 분석은 잠시 더 진행될 수 있어요. 필요하면 다시 요청해 주세요.'}</p>{canRetry && <button className="secondary-button" onClick={onRetry} disabled={busy}><Icon name="refresh" size={16}/>다시 시도</button>}</div></div> : result && <>
      ...
      {!result.chart && rows.length > 0 && (rows.length > 1 || result.status !== 'succeeded') && <section className="chart-card"><div className="chart-toolbar"><strong>조회 데이터</strong><button className="icon-button" aria-label="조회 데이터 CSV 저장" onClick={() => downloadRows(rows)}><Icon name="download" size={18}/></button></div><DataTable rows={rows}/><div className="chart-footnote"><span>{rows.length}행 표시{result.query_result?.limit_reached ? ' · 조회 상한 적용' : ''}</span><button onClick={() => onInspect('data')}>데이터 · SQL 보기<Icon name="chevron" size={14}/></button></div></section>}
      <ResultLimitations items={result.limitations}/>
      {result.data_sources?.length > 0 && <details className="more-limitations"><summary>데이터 출처 정보</summary>{result.data_sources.map((source, index) => <p key={index}>{source.label} · {source.description || source.details?.join(' ')}</p>)}</details>}

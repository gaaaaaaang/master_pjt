...
def analyze_request(question: str, **context: Any) -> IntentAnalysis:
    ...
    if len(fabs) > 1:
        missing = ["single_fab_scope"]
        clarification = "현재 한 번에 하나의 FAB을 조회합니다. 먼저 조회할 FAB을 지정해주세요."
    elif fabs and fabs[0] not in {"fab10", "fab11", "fab12", "fab13"}:
    ...
    patterns = {
        ...
        "comparison": r"비교|대비|compare|comparison|versus|\bvs\b",
        "visualization": r"그래프|차트|시각화|chart|graph|plot",
        "knowledge": r"정의|개념|기초|설명|매뉴얼|절차|what is|explain|definition|sop",
    }
